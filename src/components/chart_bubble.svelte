<script>
    ...
    onMount(() => {
        ...
        const tip = d3.tip()
            ...
            .html(d => {
                return d.sort((a,b) => d3.descending(a.percent, b.percent)).reduce((acc,cur, i) => {
                    return acc + `<p class="${'tooltip-p tooltip-color-' + i}""><strong>${dictionary[cur.key]}</strong><br />${d3.format(numberFormat)(cur.percent)} 
        ...
        typeGroup.each(function(d,i){
            var dataGroup = d3.select(this)
                .selectAll('.data-group')
                    .data(d => {
                        console.log(d);
                        return d.filter(x => !isNaN(x.percent)).sort((a,b) => d3.descending(a.percent, b.percent));
                    })
                    .enter().append('g')
