<script>
    ...
    function backToTop(){
        ...
        //scrolledToStore.set(dictionary.order[0]);
...
{#each groupedData as group, i}
    <section class="dataviz-section">
        ...
        {/if}
        <p class="description">{dictionary[group.key] && dictionary[group.key].desc ? dictionary[group.key].desc.replace(/\.$/,'') + '.' : ''} {dictionary[group.key] && (dictionary[group.key].source || dictionary[group.key].sources) ? (dictionary[group.key].source ? 'Source: ' : 'Sources: ') + ( dictionary[group.key].source || dictionary[group.key].sources ) : ''}</p>
